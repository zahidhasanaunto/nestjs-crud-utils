...
class TypeOrmCrudService extends crud_1.CrudService {
    ...
    get update() {
        return this.repo.update.bind(this.repo);
    }
    get count() {
    ...
    async createOne(req, dto) {
        ...
        if (returnShallow) {
            ...
            if (!primaryParams.length && primaryParams.some((p) => (0, util_1.isNil)(saved[p]))) {
                return saved;
            }
            else {
    ...
    async createMany(req, dto) {
        if (!(0, util_1.isObject)(dto) || !(0, util_1.isArrayFull)(dto.bulk)) {
            this.throwBadRequestException(`Empty data. Nothing to save.`);
        }
        const bulk = dto.bulk
            .map((one) => this.prepareEntityBeforeSave(one, req.parsed))
            .filter((d) => !(0, util_1.isUndefined)(d));
        if (!(0, util_1.hasLength)(bulk)) {
            this.throwBadRequestException(`Empty data. Nothing to save.`);
        }
    ...
    async updateOne(req, dto) {
        ...
        const toSave = !allowParamsOverride
            ? Object.assign(Object.assign(Object.assign(Object.assign({}, entityIdMap), dto), paramsFilters), req.parsed.authPersist) : Object.assign(Object.assign(Object.assign({}, entityIdMap), dto), req.parsed.authPersist);
        const updated = await this.repo.save((0, class_transformer_1.plainToClass)(this.entityType, toSave));
        if (returnShallow) {
            return updated;
        }
        else {
    ...
    async replaceOne(req, dto) {
        ...
        const paramsFilters = this.getParamFilters(req.parsed);
        req.options.query.cache = false;
        const [_, found] = await (0, util_1.asyncAwaitWrapper)(this.getOneOrFail(req, returnShallow));
        const toSave = !allowParamsOverride
        ...
        const replaced = await this.repo.save((0, class_transformer_1.plainToClass)(this.entityType, toSave));
        if (returnShallow) {
            return replaced;
        }
        else {
            ...
            req.parsed.search = primaryParams.reduce((acc, p) => (Object.assign(Object.assign({}, acc), { [p]: replaced[p] })), {});
            return this.getOneOrFail(req);
        }
    }
    async deleteOne(req) {
        ...
        const toReturn = returnDeleted
            ? (0, class_transformer_1.plainToClass)(this.entityType, Object.assign({}, found))
            : undefined;
    ...
    getParamFilters(parsed) {
        const filters = {};
        if ((0, util_1.hasLength)(parsed.paramsFilter)) {
    ...
    async createBuilder(parsed, options, many = true, withDeleted = false) {
        ...
        const allowedJoins = (0, util_1.objKeys)(joinOptions);
        if ((0, util_1.hasLength)(allowedJoins)) {
            const eagerJoins = {};
            ...
            if ((0, util_1.isArrayFull)(parsed.join)) {
    ...
    prepareEntityBeforeSave(dto, parsed) {
        ...
        if (!(0, util_1.hasLength)((0, util_1.objKeys)(dto))) {
            return undefined;
        }
        return dto instanceof this.entityType
            ? Object.assign(Object.assign({}, dto), parsed.authPersist) : (0, class_transformer_1.plainToClass)(this.entityType, Object.assign(Object.assign({}, dto), parsed.authPersist));
    }
    ...
    setJoin(cond, joinOptions, builder) {
        ...
        if (options.select !== false) {
            const columns = (0, util_1.isArrayFull)(cond.select)
            ...
            const select = [
                ...
                ...((0, util_1.isArrayFull)(options.persist) ? options.persist : []),
                ...columns,
            ].map((col) => `${alias}.${col}`);
    ...
    setSearchCondition(builder, search, customOperators, condition = '$and') {
        if ((0, util_1.isObject)(search)) {
            const keys = (0, util_1.objKeys)(search);
            if (keys.length) {
                if ((0, util_1.isArrayFull)(search.$and)) {
                ...
                else if ((0, util_1.isArrayFull)(search.$or)) {
                    if (keys.length === 1) {
    ...
    builderSetWhere(builder, condition, field, value, customOperators, operator = '$eq') {
        ...
        const args = [
            { field, operator: (0, util_1.isNull)(value) ? '$isnull' : operator, value },
            index,
            builder,
            customOperators,
        ];
    ...
    setSearchFieldObjectCondition(builder, condition, field, object, customOperators) {
        if ((0, util_1.isObject)(object)) {
            const operators = (0, util_1.objKeys)(object);
            if (operators.length === 1) {
                const operator = operators[0];
                const value = object[operator];
                if ((0, util_1.isObject)(object.$or)) {
                    const orKeys = (0, util_1.objKeys)(object.$or);
    ...
    checkFilterIsArray(cond, withLength) {
        if (!Array.isArray(cond.value) ||
            !cond.value.length ||
            (!(0, util_1.isNil)(withLength) ? withLength : false)) {
